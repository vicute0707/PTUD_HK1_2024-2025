...
public class ChiTietSanPham extends JDialog {
	...
	public ChiTietSanPham(Frame owner, String productID) {
...
//    }  
	...
	private void initComponents() {
		...
		txtDanhMuc.setText(product.getDanhmuc().getTenDM());
		txtTonKho = new JTextField();
		txtTonKho.setText(String.valueOf(product.getSoLuongTonKho()));
		txtGiaBan = new JTextField();
		txtGiaBan.setText(String.format("%,.0f VNĐ", product.getGiaBan()));
		txtGiaNhap = new JTextField();
		txtGiaNhap.setText(String.format("%,.0f VNĐ", product.getGiaNhap()));
		txtThuongHieu = new JTextField();
		txtThuongHieu.setText(product.getThuongHieu());
		txtMoTa = new JTextArea();
		txtMoTa.setText(product.getTinhTrang());
		imagePath = product.getHinhAnh();
		...
		txtMoTa.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(DARK_PINK),
		...
		btnPhanLoai.addActionListener(e -> {
			XemChiTietPhanLoai dialog = new XemChiTietPhanLoai(
					(Frame) SwingUtilities.getWindowAncestor(ChiTietSanPham.this),masp);
